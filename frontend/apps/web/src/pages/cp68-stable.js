...
async function initialLoad(symbol, timeframe) {
    ...
    const url = `${API_BASE_URL}/api/dataset/candles?symbol=${symbol}&limit=${CHUNK_SIZE}`; // <-- Dùng tên đã sửa

    const historyPromise = fetch(url)
        .then(res => res.json())
        .then(arr => {
            if (!Array.isArray(arr)) return []; // Tránh lỗi nếu API trả về lỗi
            return arr.map(item => {
...
function initializeSearch() {
    ...
    let allSymbolsForSearch = []; // Sử dụng một biến riêng cho tìm kiếm
    let activeIndex = -1;

    // Tải danh sách mã đầy đủ từ dataset (bao gồm cả chỉ số)
    fetch(`${API_BASE_URL}/api/dataset/symbols_with_type`)
        .then(res => res.json())
        .then(data => {
            // Lấy thêm tên đầy đủ của công ty để hiển thị
            dataProvider.getAllCompanies().then(companies => {
                const companyNameMap = new Map(companies.map(c => [c.symbol, c.organ_name]));
                allSymbolsForSearch = data.map(s => ({
                    ...s,
                    organ_name: companyNameMap.get(s.symbol) || (s.type === 'INDEX' ? `Chỉ số ${s.symbol}` : '')
                }));
                console.log(`Đã tải xong ${allSymbolsForSearch.length} mã để tìm kiếm.`);
            });
        });
    ...
    searchInput.addEventListener('input', () => {
        ...
        // Lọc trên danh sách allSymbolsForSearch thay vì allCompanies
        const filtered = allSymbolsForSearch.filter(c => {
            const symbolMatch = (c.symbol || '').toUpperCase().includes(query);
            const nameMatch = (c.organ_name || '').toUpperCase().includes(query);
            return symbolMatch || nameMatch;
        });

        suggestionsContainer.innerHTML = '';
        if (filtered.length > 0) {
            filtered.slice(0, 50).forEach(company => {
                const item = document.createElement('div');
    ...
    // ... Các event listener cho keydown, focus, blur giữ nguyên như trong file gốc ...
    // (Đoạn mã dưới đây không cần thay đổi)
    searchInput.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            const targetItem = activeIndex > -1 ? items[activeIndex] : items[0];
            if (targetItem) {
...
async function loadMoreHistory() {
    ...
    console.log("Đang tải thêm dữ liệu cũ hơn từ dataset...");
    ...
    const url = `${API_BASE_URL}/api/dataset/candles?symbol=${currentSymbol}&to=${toDateStr}&limit=${CHUNK_SIZE}`; // <-- Dùng tên đã sửa

    try {
        const olderDataRaw = await fetch(url).then(res => res.json());

        if (olderDataRaw && olderDataRaw.length > 0) {
            const olderData = olderDataRaw.map(item => {
                const d = new Date(item.date);
                return {
                    ...item,
                    time: { year: d.getFullYear(), month: d.getMonth() + 1, day: d.getDate() },
            ...
            currentCandlestickData = mergeCandles(olderData, currentCandlestickData);
            applyDataToChart(currentCandlestickData);
